...
ax = pp.axes(xlim=(minx,maxx),ylim=(miny,maxy))
MullerForce.plot(ax=ax)

scatter = ax.scatter(startingPositions[:,0], startingPositions[:,1], 
        edgecolor='none',
        c = range(100),
        cmap='Reds') #, facecolor='k')
...
#print(xx[path[:,0],0])
#ax = pp.gca()
#ax.plot(xx[path[:,1],0],yy[path[:,0],0])
pp.show()
